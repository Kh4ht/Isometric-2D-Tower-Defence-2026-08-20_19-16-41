...
{
    internal static class AutomationResultHelper
    {
        internal static AutomationResponse EnsureInit()
        {
            AI.Init();
            if (!AI.IsInitialized)
            {
                return AutomationResponse.Error("Asset Inventory is not initialized. Open the Asset Inventory window first to set up the database.", errorCode: "not_ready");
            }
            if (!DBAdapter.IsDBOpen())
            {
                return AutomationResponse.Error("Asset Inventory database is not open.", errorCode: "not_ready");
            }
            return null;
        }
        ...
        internal static object ToPackageResult(AssetInfo info)
        {
            ...
            return new
            {
                ...
                firstRelease = info.FirstRelease > DateTime.MinValue ? info.FirstRelease.ToString("yyyy-MM-dd") : null,
                lastRelease = info.LastRelease > DateTime.MinValue ? info.LastRelease.ToString("yyyy-MM-dd") : null,
                lastUpdate = info.LastUpdate > DateTime.MinValue ? info.LastUpdate.ToString("yyyy-MM-dd") : null,
                purchaseDate = info.PurchaseDate > DateTime.MinValue ? info.PurchaseDate.ToString("yyyy-MM-dd") : null,
                slug = info.Slug,
                previewImagePath = previewPath
            };
        }
