...
namespace AssetInventory
{
    ...
    public static partial class SearchIntegration
    {
        ...
        private static bool ShouldSearchDatabase(SearchContext context)
        {
            string query = context?.searchQuery ?? string.Empty;
            if (query.StartsWith(FilterPrefix)) return true;
            return (IndexUI.HasOpenInstances || AI.Config.forceInitOnDomainReload) && AI.IsInitialized && DBAdapter.IsDBOpen();
        }
        ...
        internal static SearchProvider CreateProvider()
        {
            SearchProvider provider = new SearchProvider(ProviderId, "Asset Inventory")
            {
                ...
                {
                    if (!ShouldSearchDatabase(context)) return null;
                ...
                fetchThumbnail = (item, context) =>
                {
                    ...
                    {
                        string previewPath = info.GetPreviewFile(Paths.GetPreviewFolder());
        ...
        private static async void AddAssetToScene(AssetInfo info)
        {
            ...
            try
            {
                ...
                {
                    targetPath = await Assets.CopyTo(info, AI.Config.importFolder, true, 0, false, false, true);
