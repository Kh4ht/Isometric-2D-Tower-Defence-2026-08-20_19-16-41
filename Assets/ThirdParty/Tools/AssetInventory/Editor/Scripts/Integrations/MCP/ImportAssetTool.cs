...
namespace AssetInventory
{
    public static class ImportAssetTool
    {
            ...
            Groups = new[] {"Asset Inventory/Import"})]        public static async Task<object> ImportAsset(ImportAssetParams parameters)
        {
            ...
            AssetImportResult importResult = await Assets.CopyToWithResult(
                info,
                targetFolder,
                parameters.WithDependencies,
                parameters.ScriptMode,
                logCollisionWarnings: false);
            string importedPath = importResult.ImportedPath;

            if (importResult.HasCollisions)
            {
                return Response.Error(Assets.FormatImportCollisionSummary(importResult.Collisions));
            }
