...
{
    /// <summary>
    /// Automator action step that recursively deletes a configured remote directory over FTP or
    /// SFTP while preserving explicitly excluded relative paths.
    /// </summary>
    [Serializable]
    public sealed class FTPDeleteStep : FTPActionStep
    {
        ...
        /// <summary>
        /// Connects to the configured FTP or SFTP server and starts deletion of the target
        /// directory, excluding any relative paths supplied by the action.
        /// </summary>
        /// <param name="parameters">
        /// Ordered action values for the saved connection, remote target directory, and optional
        /// comma-separated exclusion paths.
        /// </param>
        /// <returns>
        /// A task that completes after the server deletion finishes, or faults when validation,
        /// connection, or a remote file operation fails.
        /// </returns>
        /// <exception cref="ArgumentException">The target directory is empty.</exception>
        /// <exception cref="InvalidOperationException">The target resolves to the server root.</exception>
