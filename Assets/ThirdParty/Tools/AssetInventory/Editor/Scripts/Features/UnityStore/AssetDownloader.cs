...
namespace AssetInventory
{
    public sealed class AssetDownloader
    {
        ...
        private void OnDownloadSuccessful(int foreignId)
        {
            ...
            PackageDownloadCompletion.SyncPackage(_asset);
        ...
        /// <summary>
        /// Whether an observed package or download-file change has invalidated the cached state.
        /// </summary>
        public bool IsDirty { get; set; } = true;

        /// <summary>
        /// Returns true if this asset is in a stable state that doesn't need active polling
        /// unless explicitly dirtied by an observed file change.
        /// </summary>
        ...
        private void CheckState()
        {
            ...
            {
                Asset dbAsset = DBAdapter.DB.Find<Asset>(_asset.AssetId) ?? _asset.ToAsset();
                Asset reconciledAsset = PackageIdentityReconciler.AssignLocationOrMerge(dbAsset, targetFile);
                if (reconciledAsset != null)
                {
                    _asset.CopyFrom(reconciledAsset);
                }
                else
                {
