...
namespace AssetInventory
{
    public static class AssetStore
    {
        ...
        private static bool _batchOriginInitialized;
        private static MethodInfo _findAssetsMethod;
        private static Type _searchFilterType;
        private static PropertyInfo _anyWithAssetOriginProp;
        private static PropertyInfo _searchAreaProp;
        ...
        {
            return _allPackages ?? new Dictionary<string, PackageInfo>();
        }
        ...
        public static Dictionary<string, PackageInfo> GetAllPackages()
        {
            ...
            {
                foreach (KeyValuePair<string, PackageInfo> pair in _allPackages)
                {
        ...
        {
            if (_projectPackages != null) return _projectPackages;

            PackageInfo[] packages = PackageInfo.GetAllRegisteredPackages();
            _projectPackages = packages.ToDictionary(p => p.name);
            OnPackageListUpdated?.Invoke();
        ...
        public static HashSet<string> GetGuidsWithOrigin()
        {
            ...
            try
            {
                if (!_batchOriginInitialized)
                {
                    _batchOriginInitialized = true;
                    Assembly assembly = Assembly.Load("UnityEditor.CoreModule");
                    _searchFilterType = assembly.GetType("UnityEditor.SearchFilter");
                    if (_searchFilterType != null)
                    {
                        _anyWithAssetOriginProp = _searchFilterType.GetProperty("anyWithAssetOrigin", BindingFlags.Public | BindingFlags.Instance);
                        _searchAreaProp = _searchFilterType.GetProperty("searchArea", BindingFlags.Public | BindingFlags.Instance);
                        _originalTextProp = _searchFilterType.GetProperty("originalText", BindingFlags.Public | BindingFlags.Instance);
                        _clearSearchMethod = _searchFilterType.GetMethod("ClearSearch", BindingFlags.Public | BindingFlags.Instance);

                        Type adbType = assembly.GetType("UnityEditor.AssetDatabase");
                        _findAssetsMethod = adbType?.GetMethod("FindAssets", BindingFlags.NonPublic | BindingFlags.Static, null, new[] {_searchFilterType}, null);
                    }
                }

                if (_searchFilterType != null && _anyWithAssetOriginProp != null && _findAssetsMethod != null)
                {
                    object filter = Activator.CreateInstance(_searchFilterType);
                    // SearchArea.AllAssets = 0
                    _searchAreaProp?.SetValue(filter, 0);
                    _clearSearchMethod?.Invoke(filter, null);
                    _originalTextProp?.SetValue(filter, "assetorigin:");
                    _anyWithAssetOriginProp.SetValue(filter, true);

                    object result = _findAssetsMethod.Invoke(null, new[] {filter});
                    if (result is string[] guids)
                    {
                        return new HashSet<string>(guids);
                    }
                }
            }
            catch (Exception e)
            {
                Debug.LogWarning($"Batch origin scanning via SearchFilter failed, falling back to per-GUID lookup: {e.Message}");
            }
#endif
            return null;
        }
