...
namespace AssetInventory
{
    ...
    public static partial class Tagging
    {
        ...
        /// <summary>Assigns a tag to a package or indexed file when the relationship does not already exist.</summary>
        public static bool AddAssignment(int targetId, string tag, TagAssignment.Target target, bool fromAssetStore = false)
        {
            ...
            return AddAssignment(targetId, existingT, target);
        }

        /// <summary>Assigns a tag to a package or indexed file when the relationship does not already exist.</summary>
        internal static bool AddAssignment(int targetId, Tag tag, TagAssignment.Target target)
        {
            if (tag == null || tag.Id <= 0) return false;

            TagAssignment existingA = DBAdapter.DB.Find<TagAssignment>(t => t.TagId == tag.Id && t.TargetId == targetId && t.TagTarget == target);
            if (existingA != null) return false; // already added

            TagAssignment newAssignment = new TagAssignment(tag.Id, target, targetId);
        ...
        /// <summary>Assigns a tag to a package or indexed file when the relationship does not already exist.</summary>
        internal static bool AddAssignment(AssetInfo info, Tag tag, TagAssignment.Target target, bool byUser = false)
        {
            if (info == null || tag == null) return false;
            if (!AddAssignment(target == TagAssignment.Target.Asset ? info.Id : info.AssetId, tag, target)) return false;

            LoadAssignments(info);
            if (byUser && target == TagAssignment.Target.Asset && info.AssetSource == Asset.Source.AssetManager) AddRemoteTag(info, tag.Name);

            return true;
        }

        /// <summary>Assigns the named tag to each supplied package or indexed file, skipping relationships that already exist.</summary>
        ...
        /// <summary>Assigns the named tag to each supplied package or indexed file, skipping relationships that already exist.</summary>
        internal static void AddAssignments(List<AssetInfo> infos, Tag tag, TagAssignment.Target target, bool byUser = false)
        {
            if (infos == null || tag == null) return;
            ...
            infos.ForEach(info =>
            {
                TagInfo tagInfo = (target == TagAssignment.Target.Asset ? info.AssetTags : info.PackageTags)?.Find(t => t.TagId == tag.Id);
                if (tagInfo != null) return;
        ...
        /// <summary>Removes a package or file tag assignment without deleting the tag definition.</summary>
        ...
        /// <summary>Removes the named tag from the supplied indexed files while leaving package assignments unchanged.</summary>
        ...
        /// <summary>Removes the named tag from the supplied packages while leaving file assignments unchanged.</summary>
        ...
        /// <summary>Returns tags currently assigned to the selected indexed file.</summary>
        ...
        /// <summary>Returns tags currently assigned to the selected package.</summary>
        ...
        /// <summary>Persists changes to an existing tag definition and refreshes the cached tag hierarchy.</summary>
        ...
        /// <summary>Creates a tag at the supplied hierarchical path, optionally persisting it immediately.</summary>
        ...
        /// <summary>Renames the tag and updates cached tag paths for its descendants without changing assignments.</summary>
        ...
        /// <summary>Deletes the selected tag definition and its assignments while retaining descendant tag definitions.</summary>
        ...
        /// <summary>Deletes the selected tag, every descendant tag, and all of their assignments.</summary>
        ...
        /// <summary>Returns every direct and indirect child of the supplied tag identity.</summary>
        ...
        /// <summary>Reloads tag definitions and assignments from the active catalog and rebuilds their hierarchy paths.</summary>
        ...
        /// <summary>Returns the catalog identities of every direct and indirect child of the supplied tag.</summary>
