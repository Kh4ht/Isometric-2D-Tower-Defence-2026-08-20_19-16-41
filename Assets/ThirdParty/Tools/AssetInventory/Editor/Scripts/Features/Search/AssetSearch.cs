...
namespace AssetInventory
{
    ...
    public static partial class AssetSearch
    {
        ...
        /// <summary>Controls file search text, package and media filters, dimensions, tags, metadata, sorting, paging, and in-memory refinement.</summary>
        public class Options
        {
            ...
            internal int SelectedAssetFileId;
        ...
        /// <summary>Contains the current page of matching indexed files, total and pre-filter counts, execution mode, and any nonfatal search error.</summary>
        public class Result
        {
        ...
        /// <summary>Executes the configured file search and returns the current result page plus total counts and diagnostics.</summary>
        public static Result Execute(Options opt)
        {
            ...
            if (SemanticAssetSearch.TryExecute(opt, out Result semanticResult))
            {
        ...
        /// <summary>Returns the subset of supplied GUIDs that already have indexed catalog records matching the search options.</summary>
        ...
        /// <summary>Contains the parameterized SQL variants, ordered arguments, and validation error produced for an asset search.</summary>
        ...
        /// <summary>Builds the parameterized catalog query and post-filter plan for the supplied search options without executing it.</summary>
        public static QueryResult BuildQuery(Options opt)
        {
            ...
            if (opt.SelectedAssetFileId > 0)
            {
                wheres.Add("AssetFile.Id = ?");
                args.Add(opt.SelectedAssetFileId);
            }
            ...
            if (opt.SelectedCategory > 0 && opt.CategoryNames.Length > opt.SelectedCategory)
            {
                ...
                {
                    wheres.Add("(Asset.DisplayCategory = ? OR Asset.DisplayCategory like ? ESCAPE '\\')");
                    args.Add(category);
                    args.Add(EscapeLikePattern(category) + "/%");
                }
                else
                {
            ...
            if (!string.IsNullOrWhiteSpace(opt.SearchPhrase))
            {
                ...
                if (AI.Config.searchAICaptions && AI.Actions.AICaptionsEnabled) searchFields.Add("AssetFile.AICaption");
            ...
            if (rawType != null)
            {
                ...
                {
                    wheres.Add("AssetFile.Type = ?");
                    args.Add(rawType.ToLowerInvariant());
                }
            }
        ...
        /// <summary>Reports whether the selected search field supports the requested filter operator.</summary>
