...
namespace AssetInventory
{
    ...
    public static partial class ProjectAssetSearch
    {
        ...
        /// <summary>Controls project search text, asset type, extension handling, result limits, tags, metadata, and optional usage calculation.</summary>
        public class Options
        {
            ...
            /// <summary>Creates options populated from the active catalog and current Asset Inventory configuration.</summary>
            ...
            /// <summary>Creates executable search options from the supplied saved-search record.</summary>
        ...
        /// <summary>Contains project asset records matching the supplied filters and the total number of matches.</summary>
        public class Result
        {
        ...
        /// <summary>Searches the open Unity project with the configured filters and returns matching project asset records.</summary>
        public static Result Execute(Options opt)
        {
            Result result = new Result();
            _virtualIdCounter = -1;

            if (!string.IsNullOrEmpty(opt.SelectedGuid))
            {
                string selectedPath = AssetDatabase.GUIDToAssetPath(opt.SelectedGuid);
                ...
                string selectedExtension = Path.GetExtension(selectedPath);
                string selectedType = !string.IsNullOrEmpty(selectedExtension)
                    ? selectedExtension.Substring(1).ToLowerInvariant()
                    : string.Empty;
                result.Files.Add(CreateVirtualAssetInfo(opt.SelectedGuid, selectedPath, selectedFileName, selectedType));
        ...
        /// <summary>Reports whether the search options contain at least one active custom-metadata filter.</summary>
