    ...
    public static class CopyCoordinator
    {
        ...
        public static async Task<string> AcquireOrCopy(string guid, string targetFolder, Func<Task<string>> copyFunc)
        {
            ...
            lock (entry.Lock)
            {
                ...
                {
                    if (IsCachedPathValid(guid, entry.TargetPath))
                    {
        ...
        internal static bool IsCachedPathValid(string expectedGuid, string targetPath)
        {
            if (string.IsNullOrWhiteSpace(targetPath)) return false;

            string fullPath = AssetUtils.AddProjectRoot(targetPath);
            if (!File.Exists(fullPath) && !Directory.Exists(fullPath)) return false;

            string assetPath = AssetUtils.GetAssetDatabasePath(targetPath);
            if (string.IsNullOrEmpty(assetPath) || string.IsNullOrEmpty(expectedGuid)) return true;

            string actualGuid = AssetDatabase.AssetPathToGUID(assetPath);
            if (string.IsNullOrEmpty(actualGuid) && File.Exists(fullPath + ".meta"))
            {
                actualGuid = AssetUtils.ExtractGuidFromFile(fullPath + ".meta");
            }
