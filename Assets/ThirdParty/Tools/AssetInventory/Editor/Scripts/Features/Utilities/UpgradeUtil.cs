...
namespace AssetInventory
{
    ...
    public partial class UpgradeUtil : ActionProgress
    {
        ...
        public IReadOnlyList<string> PendingLongUpgrades => PendingUpgrades;
        private Vector2 _scrollPos;

        public void PerformUpgrades()
        {
            // Use elevated timeout during upgrades (important for MySQL with large databases)
            DBAdapter.DB.CommandTimeout = new DatabaseSettingsAdapter().MySqlUpgradeTimeout;
            try
            {
                PerformUpgradesInternal();
            }
            finally
            {
                DBAdapter.DB.CommandTimeout = 0;
            }
        }

        private void PerformUpgradesInternal()
        {
            ...
            if (oldVersion < 17)
            {
                // Use abstraction layer CreateIndex which handles database differences
                // Note: COLLATE NOCASE is handled by the abstraction layer (mapped to utf8mb4_general_ci for MySQL)
                DBAdapter.DB.CreateIndex("AssetFile", "Path", false);
                DBAdapter.DB.CreateIndex("AssetFile", "FileName", false);
            }
            if (oldVersion < 18)
            {
            ...
            {
                // Use abstraction layer CreateIndex which handles database differences
                DBAdapter.DB.CreateIndex("Tag", "Name", false);
                DBAdapter.DB.CreateIndex("MetadataDefinition", "Name", false);
            }
            if (oldVersion < 25)
            {
            ...
            if (oldVersion < 27)
            {
                // Convert OfficialState from string to enum (integer)
                DBAdapter.DB.RunInTransaction(() =>
                {
                    // Drop index if it exists (SQLite.net creates indexes with pattern "TableName_ColumnName")
                    try
                    {
                        DBAdapter.DB.Execute("DROP INDEX IF EXISTS Asset_OfficialState");
                    }
                    catch
                    {
                    ...
                    DBAdapter.DB.Execute("ALTER TABLE Asset RENAME COLUMN OfficialState TO OldOfficialState");
                    DBAdapter.DB.Execute("ALTER TABLE Asset ADD COLUMN OfficialState integer");
                    DBAdapter.DB.Execute("UPDATE Asset SET OfficialState = CASE WHEN OldOfficialState = 'published' THEN 1 WHEN OldOfficialState = 'deprecated' THEN 2 WHEN OldOfficialState = 'disabled' THEN 3 ELSE 0 END");
                    DBAdapter.DB.Execute("ALTER TABLE Asset DROP COLUMN OldOfficialState");
                    
                    // Index will be automatically recreated by SQLite.net when it sees [Indexed] attribute
                });
            }
            if (oldVersion < 28)
            {
                // Add covering index for Assets.Load() GROUP BY query (critical for MySQL/MariaDB performance)
                DBAdapter.DB.CreateIndex("AssetFile", new[] {"AssetId", "Size"}, false);
            }
            if (oldVersion < 29)
            {
                ConvertNoIndexTagAssignments();
            }
            ...
            {
                if (AI.Config.autoCalculateDependencies == 1) AI.Config.autoCalculateDependencies = 2;
                if (AI.Config.sortField == 0) AI.Config.sortField = 2;
            }
            if (oldConfigVersion < 4)
            {
                if (AI.Config.maxConcurrentUnityRequests < 100) AI.Config.maxConcurrentUnityRequests = 100;
            }
            if (oldConfigVersion < 5)
            {
                if (!AI.Config.excludePreviewExtensions && string.IsNullOrEmpty(AI.Config.excludedPreviewExtensions))
                {
                    AI.Config.excludePreviewExtensions = true;
                    AI.Config.excludedPreviewExtensions = "{audio}";
                    Debug.Log("Asset Inventory: Enabled audio preview exclusion by default to mitigate Unity GPU memory leak in AssetPreview.GetAssetPreview(). Disable under Settings > Previews > Exclude Extensions if needed.");
                }
            }
            if (oldConfigVersion < 6)
            {
                AI.Config.useUnityPipelineConverter = false;
            }
            if (oldConfigVersion < 7)
            {
                AI.Config.searchScope = (int)SearchScope.Index;
                AI.Config.showIndexSearchScope = true;
            }
            if (oldConfigVersion < 8)
            {
                RemoveNoIndexFromFolderAutoTags();
            }
            if (oldConfigVersion < 9)
            {
                AI.Config.packageBackupFeatureEnabled = IsConfiguredActionEnabled(AI.Config, ActionHandler.ACTION_BACKUP);
                AI.Config.aiCaptionsFeatureEnabled = IsConfiguredActionEnabled(AI.Config, ActionHandler.ACTION_AI_CAPTIONS);
                AI.Config.semanticSearchFeatureEnabled = IsConfiguredActionEnabled(AI.Config, ActionHandler.ACTION_SEMANTIC_INDEX);
                AI.Config.codeSearchFeatureEnabled = IsConfiguredActionEnabled(AI.Config, ActionHandler.ACTION_CODE_INDEX);
            }
        ...
        internal static bool IsConfiguredActionEnabled(AssetInventorySettings settings, string actionKey)
        {
            UpdateActionStates activeState = settings?.actionStates?.FirstOrDefault();
            UpdateActionState actionState = activeState?.actions?.FirstOrDefault(action => action.key == actionKey);
            return actionState?.enabled ?? false;
        }

        private static void SeedData()
        {
            AppProperty seedProp = DBAdapter.DB.Find<AppProperty>("SeedVersion");
            int seedVersion = seedProp != null ? int.Parse(seedProp.Value) : 0;
            if (seedVersion >= CURRENT_SEED_VERSION) return;

            if (seedVersion < 1)
            {
                SeedSampleAction();
                SeedDefaultPackageSearches();
                SeedDefaultSearches();
            }

            DBAdapter.DB.InsertOrReplace(new AppProperty("SeedVersion", CURRENT_SEED_VERSION.ToString()));
        }

        private static void ConvertNoIndexTagAssignments()
        {
            List<Tag> noIndexTags = DBAdapter.DB.Query<Tag>("select * from Tag where lower(Name)=?", "noindex");
            if (noIndexTags.Count == 0) return;

            DBAdapter.DB.RunInTransaction(() =>
            {
                foreach (Tag tag in noIndexTags)
                {
                    List<TagAssignment> packageAssignments = DBAdapter.DB.Query<TagAssignment>(
                        "select * from TagAssignment where TagId=? and TagTarget=?",
                        tag.Id,
                        (int)TagAssignment.Target.Package);

                    foreach (TagAssignment assignment in packageAssignments)
                    {
                        DBAdapter.DB.Execute("update Asset set NoIndex=? where Id=?", 1, assignment.TargetId);
                        DBAdapter.DB.Delete<TagAssignment>(assignment.Id);
                    }

                    int remainingAssignments = DBAdapter.DB.ExecuteScalar<int>(
                        "select count(*) from TagAssignment where TagId=?",
                        tag.Id);
                    if (remainingAssignments == 0) DBAdapter.DB.Delete<Tag>(tag.Id);
                }
            });
        ...
        private static void RemoveNoIndexFromFolderAutoTags()
        {
            if (AI.Config.folders == null) return;

            foreach (FolderSpec spec in AI.Config.folders)
            {
                if (spec == null || !spec.assignTag || string.IsNullOrWhiteSpace(spec.tag)) continue;

                string[] tags = StringUtils.Split(spec.tag, new[] {';', ','});
                List<string> keptTags = new List<string>();
                bool removedNoIndex = false;
                ...
                foreach (string rawTag in tags)
                {
                    ...
                    if (string.Equals(tag, "noindex", StringComparison.OrdinalIgnoreCase))
                    {
                        removedNoIndex = true;
                        continue;
                    }
                ...
                if (!removedNoIndex) continue;

                spec.tag = string.Join(", ", keptTags);
                if (keptTags.Count == 0)
                {
                    spec.assignTag = false;
                    spec.tag = null;
                }
            }
        }

        private static void SeedDefaultPackageSearches()
        {
            if (DBAdapter.DB.Table<SavedPackageSearch>().Count() > 0) return;

            DBAdapter.DB.Insert(new SavedPackageSearch {Name = "All", Color = "5B9BD5", PackagesListing = 1});
            DBAdapter.DB.Insert(new SavedPackageSearch {Name = "Recently Purchased", Color = "3498DB", PackagesListing = 1, PurchaseDateOption = 3});
            DBAdapter.DB.Insert(new SavedPackageSearch {Name = "Recently Updated", Color = "70AD47", PackagesListing = 1, UpdateDateOption = 3});
            DBAdapter.DB.Insert(new SavedPackageSearch {Name = "Update Available", Color = "ED7D31", PackagesListing = 1, Maintenance = 2});
            DBAdapter.DB.Insert(new SavedPackageSearch {Name = "Asset Store Only", Color = "9B59B6", PackagesListing = 2});
        }

        private static void SeedDefaultSearches()
        {
            if (DBAdapter.DB.Table<SavedSearch>().Count() > 0) return;

            DBAdapter.DB.Insert(new SavedSearch
            {
                Name = "Sound Effects",
                Color = "4ECDC4",
                Type = "Audio",
        ...
        private static void SeedSampleAction()
        {
            CustomAction action = new CustomAction("Hello World");
            action.Description = "A sample action demonstrating variables, debug logging, and message dialogs. Feel free to edit or delete it.";
            DBAdapter.DB.Insert(action);

            CustomActionStep step1 = new CustomActionStep();
            step1.ActionId = action.Id;
            step1.Key = "SetTextVariable";
            step1.OrderIdx = 0;
            step1.Values = new List<ParameterValue>
            {
                new ParameterValue("greeting"),
                new ParameterValue("Hello from Asset Inventory!")
            };
            step1.PersistValues();
            DBAdapter.DB.Insert(step1);

            CustomActionStep step2 = new CustomActionStep();
            step2.ActionId = action.Id;
            step2.Key = "DebugLog";
            step2.OrderIdx = 1;
            step2.Values = new List<ParameterValue>
            {
                new ParameterValue("$greeting")
            };
            step2.PersistValues();
            DBAdapter.DB.Insert(step2);

            CustomActionStep step3 = new CustomActionStep();
            step3.ActionId = action.Id;
            step3.Key = "MessageDialog";
            step3.OrderIdx = 2;
            step3.Values = new List<ParameterValue>
            {
                new ParameterValue("Custom Action"),
                new ParameterValue("$greeting\n\nThis is a sample action demonstrating the custom actions feature.\nYou can edit or delete it in the Settings tab.")
            };
            step3.PersistValues();
            DBAdapter.DB.Insert(step3);
        }
        ...
        private void RenameExtractedFolders()
        {
            ...
            foreach (Asset asset in assets)
            {
                ...
                try
                {
                    string expectedPath = Paths.GetMaterializedAssetPath(asset);
        ...
        {
            return IOUtils.PathCombine(Paths.GetMaterializeFolder(), asset.SafeName);
        }

        public async void StartLongRunningUpgrades()
        {
        ...
        private async Task UpgradeCacheFolderVersions()
        {
            ...
            try
            {
                string[] dirs = Directory.GetDirectories(Paths.GetMaterializeFolder());
                ...
                foreach (string dir in dirs)
                {
                    ...
                    string newDir = Path.Combine(Paths.GetMaterializeFolder(), newName);
        ...
        private async Task UpgradeCustomPackageDates()
        {
            ...
            foreach (Asset asset in assets)
            {
                ...
                string loc = Paths.DeRel(asset.Location);
        ...
        private void UpgradeAssetCashLocation()
        {
            ...
            string oldPrefix = Paths.GetAssetCacheFolder();
            string newPrefix = "[ac]";
            ...
            oldPrefix = Paths.GetPackageCacheFolder();
            newPrefix = "[pc]";
            affected = DBAdapter.DB.Execute("UPDATE Asset SET Location = REPLACE(Location, ?, ?) WHERE Location LIKE ?", oldPrefix, newPrefix, oldPrefix + "%");
        ...
        [Obsolete("The Asset Inventory main window renders upgrades through its UI Toolkit blocker. This IMGUI compatibility method will be removed in the next major version.")]
        public void DrawUpgradeRequired()
        {
            ...
            EditorGUILayout.LabelField("A longer or incompatible database upgrade is required for this version.", CommonUIStyles.whiteCenter);
            ...
            if (GUILayout.Button("Start Upgrade Process", GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT)))
            {
            ...
            {
                EditorGUILayout.Space();
                EditorGUILayout.LabelField(CurrentMain, CommonUIStyles.whiteCenter);
                EditorGUILayout.Space();
                CommonUIStyles.DrawProgressBar(MainProgress / (float)MainCount, CurrentSub);
            }
