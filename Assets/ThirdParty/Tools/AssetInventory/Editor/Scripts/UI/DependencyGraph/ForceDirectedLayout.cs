    ...
    public class ForceDirectedLayout
    {
        ...
        public void InitializePackagePositions(DependencyGraphData graphData)
        {
            ...
            for (int i = 0; i < graphData.Packages.Count; i++)
            {
                ...
                // Deterministic jitter breaks symmetry without making the same graph jump between openings.
                uint hash = unchecked((uint)package.AssetId * 2654435761u);
                float jitterX = ((hash & 255u) / 255f - 0.5f) * 100f;
                float jitterY = (((hash >> 8) & 255u) / 255f - 0.5f) * 100f;
                package.Position += new Vector2(jitterX, jitterY);
