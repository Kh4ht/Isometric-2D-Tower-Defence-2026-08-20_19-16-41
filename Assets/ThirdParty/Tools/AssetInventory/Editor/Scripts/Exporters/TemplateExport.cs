...
namespace AssetInventory
{
    ...
    public sealed partial class TemplateExport : ActionProgress
    {
        ...
        /// <summary>Renders the selected packages through the supplied template hierarchy and writes the resulting files to the configured target.</summary>
        public async Task Run(List<AssetInfo> assets, TemplateInfo template, List<TemplateInfo> templates, TemplateExportSettings settings, TemplateExportEnvironment env)
        {
            ...
            _assets.ForEach(asset =>
            {
                ...
                {
                    string file = asset.ToAsset().GetPreviewFile(Paths.GetPreviewFolder());
        ...
        /// <summary>Loads and merges parent templates into the supplied template, resolving relative assets from the target folder.</summary>
        ...
        {
            if (!await Task.Run(() => CompressionUtil.ExtractArchive(path, targetFolder)))
            {
