...
{
    /// <summary>Importer for discovering, indexing, and persisting asset manager content in the Asset Inventory catalog.</summary>
    public sealed class AssetManagerImporter : AssetImporter
    {
        /// <summary>Synchronizes configured Unity Cloud Asset Manager projects and collections into the local catalog, optionally limiting work to one existing package.</summary>
                ...
                foreach (IOrganization organization in cam.AvailableOrganizations)
                {
                    ...
                    for (int i = 0; i < cam.AvailableProjects.Count; i++)
                    {
                        ...
                        if (projectAlt != null)
                        {
                            ...
                            {
                                string previewFile = asset.GetPreviewFile(Paths.GetPreviewFolder(), false);
                        ...
                        await AI.Actions.RunWithProgress<AssetManagerImporter>(
                            ActionHandler.ACTION_ASSET_MANAGER_COLLECTION_INDEX,
                            "Indexing Collections",
                            imp => imp.IndexCollections(cam, asset));
                    }
                }
            }
#else
...
#if USE_ASSET_MANAGER && USE_CLOUD_IDENTITY
        /// <summary>Creates or updates catalog file records for the supplied Asset Manager assets and optionally removes records no longer present remotely.</summary>
        public async Task PersistAssetFiles(CloudAssetManagement cam, List<IAsset> assets, Asset asset, bool checkOrphans = true)
        {
            ...
            {
                orphanedFiles.ForEach(Assets.ForgetAssetFile);
            }
            if (tagsChanged)
            {
                Tagging.LoadTags();
        ...
        private async Task IndexCollections(CloudAssetManagement cam, Asset parent)
        {
            ...
            orphaned.ForEach(a => Assets.RemovePackage(new AssetInfo(a), false));
        }

        private async void FetchPreview(IAsset cloudAsset, AssetFile af)
        {
            if (af.PreviewState == AssetFile.PreviewOptions.Error) return;
            string targetFile = af.GetPreviewFile(Paths.GetPreviewFolder());
