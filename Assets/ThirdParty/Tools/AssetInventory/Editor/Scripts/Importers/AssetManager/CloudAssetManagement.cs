...
namespace AssetInventory
{
    ...
    public class CloudAssetManagement
    {
        ...
        public async Task<IAsset> CreateAssetAsync(AssetType assetType, string name, string description = null, List<string> tags = null, Dictionary<string, MetadataValue> metadata = null)
        {
            ...
            using (CancellationTokenSource cancellationTokenSrc = new CancellationTokenSource(k_DefaultCancellationTimeout))
            {
                try
                {
        ...
        public async Task<IFile> UploadFile(IDataset dataset, string filePath, string folderPath = null, string description = null)
        {
            ...
            string path = folderPath == null ? Path.GetFileName(filePath) : IOUtils.GetRelativePath(folderPath, filePath);
        ...
        {
            m_CurrentQuery = CurrentProject.QueryAssetVersions(CurrentAsset.Descriptor.AssetId).OrderBy(sortingField, sortingOrder);
