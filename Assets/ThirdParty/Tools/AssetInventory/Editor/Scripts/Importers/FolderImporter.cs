...
{
    /// <summary>Indexes configured filesystem folders and their contents as Asset Inventory sources.</summary>
    public sealed class FolderImporter : AssetImporter
    {
        /// <summary>Scans configured additional folders and refreshes their package and file records, optionally forcing unchanged sources to be reindexed.</summary>
                ...
                switch (spec.folderType)
                {
                    case 0:
                        ...
                        await AI.Actions.RunWithProgress<UnityPackageImporter>(
                            ...
                            async imp =>
                            {
                                HashSet<int> detailCandidateIds = await imp.IndexRoughLocalAndCollectDetailCandidates(spec, hasAssetStoreLayout, force);
                                if (AI.Config.indexAssetPackageContents)
                                {
                                    imp.RestartProgress("Indexing package contents");
                                    await imp.IndexDetails(detailCandidateIds);
                                }
                            });
                        break;
