...
{
    /// <summary>Indexes supported archive files and nested archives as Asset Inventory package records.</summary>
    public sealed class ArchiveImporter : AssetImporter
    {
        private const int BREAK_INTERVAL = 30;

        /// <summary>Scans the configured source folder for supported archives and creates or refreshes their package records.</summary>
        public async Task Run(FolderSpec spec)
        {
            if (string.IsNullOrEmpty(spec.location)) return;

            string scanRoot = spec.GetLocation(true);
            string[] files = IOUtils.GetFiles(scanRoot, new[] {"*.zip", "*.rar", "*.7z"}, SearchOption.AllDirectories).ToArray();
        ...
        /// <summary>Extracts and indexes the contents of the supplied archive package, updating its files and completion state.</summary>
        ...
        {
            string tempPath = await Assets.Extract(asset, ct: AI.Actions.CancellationToken);
        ...
        /// <summary>Creates child package records for supported archives discovered inside the indexed parent archive.</summary>
        public async Task ProcessSubArchives(Asset asset, List<AssetFile> subArchives)
        {
            ...
            if (AI.Config.indexSubPackages && subArchives.Count > 0)
            {
                ...
                for (int i = 0; i < subArchives.Count; i++)
                {
                    ...
                    string path = await Assets.EnsureMaterialized(asset, subArchive, ct: AI.Actions.CancellationToken);
                    if (path == null)
                    {
                    ...
                    // Skip sub-archives marked as no-index (inherits from parent)
