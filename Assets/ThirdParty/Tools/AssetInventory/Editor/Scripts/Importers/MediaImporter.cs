...
{
    /// <summary>Importer for discovering, indexing, and persisting media content in the Asset Inventory catalog.</summary>
    public sealed class MediaImporter : AssetImporter
    {
        ...
        /// <summary>Indexes supported media from a source folder, optionally attaching it to an existing package and storing relative paths.</summary>
        public async Task Index(FolderSpec spec, Asset attachedAsset = null, bool storeRelativePath = false, bool actAsSubImporter = false, bool skipSubPackages = false)
        {
            ...
            if (attachedAsset.SafeName != Asset.NONE && HasNoIndex(attachedAsset))
            {
                MarkDone(attachedAsset);
                if (!CancellationRequested) ApplyPackageTags(spec, attachedAsset);
                return;
            }
