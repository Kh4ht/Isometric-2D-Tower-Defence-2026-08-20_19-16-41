...
{
    /// <summary>Importer for discovering, indexing, and persisting unity package content in the Asset Inventory catalog.</summary>
    public sealed class UnityPackageImporter : AssetImporter
    {
        ...
        /// <summary>Discovers Unity package archives in the configured source and creates or refreshes their package records.</summary>
        public async Task IndexRoughLocal(FolderSpec spec, bool fromAssetStore, bool force = false)
        {
            await IndexRoughLocalAndCollectDetailCandidates(spec, fromAssetStore, force);
        }
        ...
        internal async Task<HashSet<int>> IndexRoughLocalAndCollectDetailCandidates(FolderSpec spec, bool fromAssetStore, bool force, Asset.Source? sourceOverride, SearchOption searchOption)
        {
            ...
            for (int i = 0; i < packages.Length; i++)
            {
                ...
                if (IsIgnoredPathBelowScanRoot(package, scanRoot)) continue;
                ...
                if (IsPendingDetailIndexCandidate(asset)) detailCandidateIds.Add(asset.Id);
            }
        ...
        /// <summary>Infers built-in, URP, and HDRP compatibility from the package name when explicit metadata is unavailable.</summary>
        ...
        /// <summary>Parses Synty package naming conventions into group, product name, minimum Unity version, and package version components.</summary>
        ...
        public static void ApplyHeader(AssetHeader header, Asset asset)
        {
            ...
            // only override if not set yet, as otherwise we loose correct metadata, header info is often outdated/incorrect
            if (string.IsNullOrWhiteSpace(asset.DisplayName) && !string.IsNullOrWhiteSpace(header.title)) asset.DisplayName = header.title;
            if (string.IsNullOrWhiteSpace(asset.Description) && !string.IsNullOrWhiteSpace(header.description)) asset.Description = header.description;
            if (string.IsNullOrWhiteSpace(asset.DisplayPublisher) && header.publisher != null) asset.DisplayPublisher = header.publisher.label;
            if (string.IsNullOrWhiteSpace(asset.DisplayCategory) && header.category != null) asset.DisplayCategory = header.category.label;
        }

        /// <summary>Extracts and indexes files for one package or all pending Unity package records.</summary>
        public Task IndexDetails(int assetId = 0)
        {
            return IndexDetails(assetId, null);
        }

        /// <summary>Extracts and indexes files for one package or all pending Unity package records.</summary>
        internal Task IndexDetails(IReadOnlyCollection<int> scopedAssetIds)
        {
            return IndexDetails(0, scopedAssetIds);
        }

        /// <summary>Extracts and indexes files for one package or all pending Unity package records.</summary>
        private async Task IndexDetails(int assetId, IReadOnlyCollection<int> scopedAssetIds)
        {
            List<Asset> assets = assetId == 0
                ? LoadPendingDetailIndexCandidates(scopedAssetIds)
                : LoadExactDetailIndexCandidate(assetId);
        ...
        internal static List<Asset> LoadPendingDetailIndexCandidates(IReadOnlyCollection<int> scopedAssetIds = null)
        {
            HashSet<int> scopedIdSet = null;
            if (scopedAssetIds != null)
            {
                scopedIdSet = new HashSet<int>(scopedAssetIds);
                if (scopedIdSet.Count == 0) return new List<Asset>();
            }
        ...
        {
            if (AI.Config.quickIndexingDone) return assets;

            List<Asset> prefAssets = assets
                .Where(IsQuickIndexPreferred)
                .Take(QUICK_INDEX_COUNT)
                .ToList();
            List<Asset> otherAssets = assets
                .Where(asset => !IsQuickIndexPreferred(asset))
                .Take(QUICK_INDEX_COUNT - prefAssets.Count)
                .ToList();
            return prefAssets.Concat(otherAssets).ToList();
        }

        private static bool IsQuickIndexPreferred(Asset asset)
        {
            return asset.SafeName != null && asset.SafeName.IndexOf(QUICK_INDEX_PREFERENCE, StringComparison.OrdinalIgnoreCase) >= 0;
        }

        private static bool IsPendingDetailIndexCandidate(Asset asset)
        {
            return asset.Id > 0
                   && !asset.Exclude
        ...
        private async Task<bool> IndexPackage(Asset asset, int progressId)
        {
            ...
            tempPath = await Assets.Extract(asset, ct: AI.Actions.CancellationToken);
            ...
            for (int i = 0; i < assets.Length; i++)
            {
                ...
                if (AI.Config.extractPreviews && File.Exists(previewFile))
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (PreviewManager.IsDefaultIcon(previewFile)) af.PreviewState = AssetFile.PreviewOptions.Redo;
                        }
                    }
                    catch (Exception e)
                    {
        ...
        /// <summary>Creates child package records for Unity packages or archives discovered inside the indexed parent package.</summary>
        public async Task ProcessSubPackages(Asset asset, List<AssetFile> subPackages)
        {
            ...
            if (AI.Config.indexSubPackages && subPackages.Count > 0)
            {
                ...
                for (int i = 0; i < subPackages.Count; i++)
                {
                    ...
                    string path = await Assets.EnsureMaterialized(asset, subPackage, ct: AI.Actions.CancellationToken);
                    if (path == null)
                    {
                    ...
                    // Skip sub-packages marked as no-index (inherits from parent)
                    if (HasNoIndex(subAsset))
                    {
                        MarkDone(subAsset);
                        continue;
                    }
        ...
        /// <summary>Reads Asset Store metadata from a Unity package header, returning null for missing or invalid archives.</summary>
