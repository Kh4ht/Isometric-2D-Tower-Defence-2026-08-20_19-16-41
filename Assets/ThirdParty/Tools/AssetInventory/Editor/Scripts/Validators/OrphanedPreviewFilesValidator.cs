...
{
    /// <summary>Finds preview files that are no longer referenced by an indexed file.</summary>
    public sealed class OrphanedPreviewFilesValidator : Validator
    {
        ...
        private async Task<List<string>> GatherOrphanedPreviews()
        {
            List<string> result = new List<string>();
            List<string> files = IOUtils.GetFiles(Paths.GetPreviewFolder(), new[]
            {
                "af*-*.png" // af & afa
        ...
        private async Task RemoveOrphanedPreviews(List<string> files)
        {
            int progress = 0;
            int count = files.Count;
