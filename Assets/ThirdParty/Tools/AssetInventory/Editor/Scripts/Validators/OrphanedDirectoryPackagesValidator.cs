...
{
    /// <summary>Finds directory-source packages that are no longer covered by a configured source folder.</summary>
            ...
            foreach (FolderSpec spec in AI.Config.folders)
            {
                ...
                switch (spec.packageMode)
                {
                    case 0: // Root Folder - exact match
                        if (string.Equals(normalizedDirPath, normalizedSpecPath, StringComparison.OrdinalIgnoreCase))
                        {
                            return true;
                        }
                        break;
