...
{
    /// <summary>Finds Unity packages and supported archives nested inside indexed packages but not yet indexed as child packages.</summary>
    public sealed class UnindexedSubPackagesValidator : Validator
    {
        ...
        public override async Task Validate()
        {
            ...
            List<AssetInfo> assets = Assets.Load().Where(a => a.ParentId > 0).ToList();
            List<AssetInfo> candidates = DBAdapter.DB.Query<AssetInfo>("select *, AssetFile.Id as Id from AssetFile left join Asset on Asset.Id = AssetFile.AssetId where Asset.AssetSource != ? AND Asset.SafeName != ? AND (Type = 'unitypackage' OR Type = 'zip' OR Type = 'rar' OR Type = '7z')", Asset.Source.AssetManager, Asset.NONE);
