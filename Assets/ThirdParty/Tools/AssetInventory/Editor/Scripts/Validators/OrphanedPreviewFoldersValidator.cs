...
{
    /// <summary>Finds preview cache folders that are no longer referenced by the catalog.</summary>
    public sealed class OrphanedPreviewFoldersValidator : Validator
    {
        ...
        private async Task<List<string>> GatherOrphanedFolders()
        {
            ...
            if (!Directory.Exists(Paths.GetPreviewFolder())) return result;

            string[] folders = Directory.GetDirectories(Paths.GetPreviewFolder());
