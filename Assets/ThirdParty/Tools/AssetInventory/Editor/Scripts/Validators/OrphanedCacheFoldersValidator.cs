...
{
    /// <summary>Finds package cache folders that are no longer referenced by the catalog.</summary>
    public sealed class OrphanedCacheFoldersValidator : Validator
    {
        ...
        private async Task<List<string>> GatherOrphanedFolders()
        {
            ...
            if (!Directory.Exists(Paths.GetMaterializeFolder())) return result;

            string[] folders = Directory.GetDirectories(Paths.GetMaterializeFolder());
        ...
        private async Task RemoveOrphanedFolders(List<string> folders)
        {
            ...
            foreach (string folder in folders)
            {
                ...
                if (Directory.Exists(folder)) await IOUtils.DeleteFileOrDirectory(folder);
            }
