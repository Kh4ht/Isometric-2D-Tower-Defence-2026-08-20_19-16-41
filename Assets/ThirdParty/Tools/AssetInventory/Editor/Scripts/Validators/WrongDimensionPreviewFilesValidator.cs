...
{
    /// <summary>Finds preview images larger than the dimensions currently configured for the preview cache.</summary>
    public sealed class WrongDimensionPreviewFilesValidator : Validator
    {
        ...
        private async Task<List<AssetInfo>> CheckPreviewDimensions()
        {
            ...
            string query = "select * from AssetFile where (PreviewState = ? or PreviewState = ? or PreviewState = ?) and Type in (\"" + typeStr + "\") and (Width >= ? or Height >= ?)";
            List<AssetInfo> files = DBAdapter.DB.Query<AssetInfo>(query, AssetFile.PreviewOptions.Provided, AssetFile.PreviewOptions.Custom, AssetFile.PreviewOptions.UseOriginal, previewSize, previewSize).ToList();
