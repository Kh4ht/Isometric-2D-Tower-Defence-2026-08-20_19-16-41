...
namespace AssetInventory
{
    ...
    public static partial class PreviewAssetManager
    {
        ...
        private const string UrpShaderGraphTarget = "UnityEditor.Rendering.Universal.ShaderGraph.UniversalTarget";
        private const string HdrpShaderGraphTarget = "UnityEditor.Rendering.HighDefinition.ShaderGraph.HDTarget";
        private const string BirpShaderGraphTarget = "ShaderGraph.BuiltInTarget";
            ...
            try
            {
                ...
                foreach (AssetInfo info in files)
                {
                    ...
                    foreach (AssetFile dep in info.Dependencies)
                    {
                        ...
                        string dependencyKey = !string.IsNullOrEmpty(dep.Guid) ? dep.Guid : $"path:{dep.AssetId}_{dep.Path}";
                        if (processedGuids.Contains(dependencyKey)) continue;
                        if (!ShouldImportPreviewDependency(dep))
                        {
                            ...
                            processedGuids.Add(dependencyKey);
                            continue;
                        }
            ...
            HashSet<string> skipGuids = null, bool selectiveMode = false)
        {
            ...
            foreach (AssetFile af in packageFiles)
            {
                ...
                if (IsShaderGraph(af) && !await ShouldImportShaderGraphForPreviewMaterialization(sourcePath))
                {
                    CacheExternalMaterializedPath(assetId, af, sourcePath, cache);
                    continue;
                }
        ...
        internal static bool ShouldImportForPreviewMaterialization(AssetFile file, bool selectiveMode)
        {
            ...
            if (selectiveMode && !DependencyAnalysis.NeedsScan(file.Type) && !GltfSupport.IsGltfType(file.Type)) return false;
            return true;
        }
        ...
        internal static bool ShouldImportShaderGraphForPreviewMaterialization(string content, bool isOnURP, bool isOnHDRP)
        {
            if (string.IsNullOrEmpty(content)) return true;

            bool hasUrpTarget = content.IndexOf(UrpShaderGraphTarget, StringComparison.Ordinal) >= 0;
            bool hasHdrpTarget = content.IndexOf(HdrpShaderGraphTarget, StringComparison.Ordinal) >= 0;
            bool hasBirpTarget = content.IndexOf(BirpShaderGraphTarget, StringComparison.Ordinal) >= 0;

            if (isOnURP) return !(hasHdrpTarget && !hasUrpTarget);
            if (isOnHDRP) return !(hasUrpTarget && !hasHdrpTarget);
            return !((hasUrpTarget || hasHdrpTarget) && !hasBirpTarget);
        }

        private static async Task<bool> ShouldImportShaderGraphForPreviewMaterialization(string sourcePath)
        {
            try
            {
                string content = await File.ReadAllTextAsync(sourcePath);
                return ShouldImportShaderGraphForPreviewMaterialization(content, AssetUtils.IsOnURP(), AssetUtils.IsOnHDRP());
            }
            catch
            {
                return true;
        ...
        private static bool IsShaderGraph(AssetFile file)
        {
            return string.Equals(file?.Type?.TrimStart('.'), "shadergraph", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static async Task CacheExternalDependencyPath(int assetId, Asset asset, AssetFile file, DependencyResultCache cache)
        {
            ...
            CacheExternalMaterializedPath(assetId, file, sourcePath, cache);
        }

        private static void CacheExternalMaterializedPath(int assetId, AssetFile file, string sourcePath, DependencyResultCache cache)
        {
            if (file == null || cache == null) return;
            if (string.IsNullOrEmpty(sourcePath)) return;
            if (string.IsNullOrEmpty(file.Guid) && string.IsNullOrEmpty(file.Path)) return;
