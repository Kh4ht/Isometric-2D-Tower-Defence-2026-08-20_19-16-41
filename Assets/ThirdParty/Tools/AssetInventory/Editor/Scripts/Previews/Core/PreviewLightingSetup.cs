    ...
    public static class PreviewLightingSetup
    {
        ...
        public static void SetLightIntensity(Light light, bool isSecondary)
        {
            ...
            if (AssetUtils.IsOnHDRP())
            {
                ...
                SetHdrpLightIntensity(light, AI.Config.cpLightIntensityHDRP * intensityMultiplier);
#else
...
#if USE_HDRP && UNITY_6000_4_OR_NEWER
        static void SetHdrpLightIntensity(Light light, float luxIntensity)
        {
            UnityEngine.Rendering.LightUnit nativeUnit = UnityEngine.Rendering.LightUnitUtils.GetNativeLightUnit(light.type);
            light.lightUnit = nativeUnit;
            light.intensity = UnityEngine.Rendering.LightUnitUtils.ConvertIntensity(
                light,
                luxIntensity,
                UnityEngine.Rendering.LightUnit.Lux,
                nativeUnit);
        }
#endif
