...
namespace Brain
{
    ...
    internal sealed class OllamaClient : IDisposable
    {
        ...
        public async Task<OllamaEmbedResponse> EmbedAsync(OllamaEmbedRequest request, CancellationToken ct = default)
        {
            string body = JsonConvert.SerializeObject(request);
            using (StringContent content = new StringContent(body, Encoding.UTF8, "application/json"))
            using (HttpResponseMessage resp = await _http.PostAsync("api/embed", content, ct).ConfigureAwait(false))
            {
                resp.EnsureSuccessStatusCode();
                string json = await resp.Content.ReadAsStringAsync().ConfigureAwait(false);
                return JsonConvert.DeserializeObject<OllamaEmbedResponse>(json);
            }
        }
