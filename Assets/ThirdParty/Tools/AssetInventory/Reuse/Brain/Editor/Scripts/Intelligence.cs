...
namespace Brain
{
    ...
    internal static partial class Intelligence
    {
        ...
        public const string OLLAMA_EMBEDDING_LIBRARY = "https://ollama.com/search?c=embedding";
        public const string OLLAMA_SERVICE_URL = "http://localhost:11434";

        public const string LMSTUDIO_WEBSITE = "https://lmstudio.ai";
        public const string LMSTUDIO_LIBRARY = "https://lmstudio.ai/models";
        ...
        {
            bool ownsDownloadState = ReferenceEquals(OllamaDownloadToken, downloadToken);

            if (ownsDownloadState)
            {
                OllamaDownloadToken = null;
                DownloadingModel = false;
            }
        ...
        public static async Task PullOllamaModelAsync(string name, Action<DownloadStatus> statusCallback, CancellationToken cancellationToken = default)
        {
            ...
            CancellationTokenSource localDownloadToken = new CancellationTokenSource();
            OllamaDownloadToken = localDownloadToken;

            try
            {
                OllamaClient ollama = GetOllamaClient();
                using (CancellationTokenSource linked = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, localDownloadToken.Token))
                {
            ...
            {
                ReleaseOllamaDownloadToken(localDownloadToken);
            }
        }
        ...
        public static bool IsLMStudioInstalled
        {
            get
            {
                if (_lmstudioInstalled == null)
                {
                    ...
                    {
                        Debug.LogWarning($"LM Studio availability check failed. Restart Unity if the backend should be available: {e.Message}");
        ...
        private static async Task CheckLMStudioInstalled()
        {
            ...
            {
                Debug.LogWarning($"LM Studio is not available: {e.Message}");
            }
        }
    ...
    [Serializable]
    internal sealed class LMStudioInfoResponse
    {
    ...
    [Serializable]
    internal sealed class LMStudioContent
    {
        public string type;
        public string text;
    ...
    [Serializable]
    internal sealed class LMStudioImageUrl
    {
        public string url;
    }
