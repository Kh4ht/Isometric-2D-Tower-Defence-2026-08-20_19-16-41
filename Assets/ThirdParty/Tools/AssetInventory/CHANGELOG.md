...
- Increase Asset Inventory tag popup height
- Restore search sub-package navigation
- Fix double click to import not working for audio anymore
- Fix Audio Tools welcome for embedded installs

## [4.7.0] - 2026-07-30

- Rebuilt the main workspace, settings, browsers, inspectors, dialogs, result trees, and dependency views with a responsive UI Toolkit interface.
- Make package backups, AI captions, semantic asset search, and code search explicitly enableable in their own settings, while keeping update scheduling and per-package choices independent.
- Keep Search filters open beside the results by choosing Filters in the left filter browser.
- Let package covers grow up to their full native resolution, and browse Asset Store gallery images with adjustable sizing and previous/next controls.
- Navigate Add Tag search results with Up and Down, then press Return to assign the highlighted tag.
- Recognize PSB files as images and offer one-click installation of Unity's 2D PSD Importer for importing them and recreating their previews.
- Show No Index packages with a muted dash directly in the Indexed column and keep them separate from new unindexed packages when sorting.
- Keep custom Packages, Search, and Reporting column widths and layouts intact after restarting Unity or resizing a window.
- Restore cached Asset Store packages to their Store source when they reappear in the current account's purchases, including older cached filenames, while leaving unowned cache files as custom packages.
- Preserve TextMesh Pro materials during render-pipeline conversion, and add a maintenance repair for materials previously changed to an SRP Lit shader.
- Limit automatic custom material conversion after package imports to the assets imported in that batch.
- Open any dependency directly in Asset Inventory from the dependency graph or list.
- Keep prefabs dragged into a scene together as siblings instead of nesting each new object under the previous one.
- Fixed package exports omitting DLLs and other files when all file types were selected.
- Preserve same-name imports with automatic renaming, with optional warn-and-skip or overwrite behavior in Import settings.
- Keep Import and Ping actions accurate when a same-name asset replaces a previously imported file.
- Remember whether the Settings statistics section is expanded after restarting Unity.
- Require Unity 2022.3 or newer.
- Open links directly inside package descriptions and release notes instead of repeating them as buttons below the text.
- Keep long Search tile names readable by limiting them to two lines with an ellipsis.
- Refine Search and Packages grid spacing with slimmer tile borders, quieter panel dividers, aligned results, and pagination that stays together.
- Choose which status pills appear on package tiles, with deprecated and disabled store packages highlighted by default.
- Added a polished guided inspector to the search example, with clearer setup, help, and actions.
- Reload the latest shared database data on demand from a configurable toolbar button or the public API.
- Keep the Settings page focused by expanding only one configuration section at a time.
- Remove selected file type groups by clicking them again in the Code and Semantic file type menus.
- Keep Asset Inventory responsive after script reloads while package download states refresh in the background.
- Prevent Unity from hanging while large Asset Store caches are monitored or other tools start file watchers, while keeping package and download status changes responsive.
- Improve additional-folder setup with clearer folder type labels, aligned choices, a roomier wizard, and cancellable background scanning for large folders.
- Keep the Additional Folders list at its current scroll position when changing folder selections or using the All, Invert, and None controls.
- Recover readable ZIP archives whose directory metadata is inconsistent, matching files Windows Explorer can still open.

## [4.6.0] - 2026-07-12

- Use the toolbar eye icon as the single control for switching between Standard and Advanced mode
...
- Keep search tile spacing stable while resizing the details panel, and move tile detail presets into View & Display settings
- Keep Search footer controls on one line whenever space remains, shifting centered paging and scope controls only as the result area narrows
- Make the selected saved search more prominent and keep AI caption commands with the caption they edit
- Restore compact download-first icons on Search actions instead of overflowing explanatory button text
- Hide inspector sections automatically when they contain no visible content
- Clearly separate selected-file details from containing-package details in the Search inspector
- Refresh saved-search colors, names, and icons immediately after editing
- Restore native Unity table resizing, Resize to Fit, horizontal scrolling, and column reordering across Search, Packages, and Reporting, and allow sorting directly from supported Search headers
